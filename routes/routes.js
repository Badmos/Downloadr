...
router.route('/')
    .get((req, res) => {
        res.render('index', { title: 'Downloadr: Download YouTube Videos in a click' })
    });
        ...
        axios.get(videoUrl)
            .then((response) => {
                ...
                if (pattern.test(videoUrl)) {
                    ...
                    youtubeDl.getInfo(videoUrl, ['--youtube-skip-dash-manifest'], (err, info) => {
                        if (err) return res.render('videos', { error: 'Invalid or unacceptable link. Kindly paste the correct Link to the YouTube Video' });
                        // add format to array. Do not include audio
                        info.formats.forEach((item) => {
                            if (item.format_note !== 'DASH audio' && item.filesize) {
                                item.filesize = item.filesize ? byteToDiskSpace(item.filesize) : 'File is too Large';
                                fileFormats.push(item);
                            }
                        });
                        res.render('videos', { error: err, meta: { id: info.id, fileFormats } });

                    })
                } else {
                    res.render('videos', { error: 'Invalid or unacceptable link. Kindly paste the correct Link to the YouTube Video' });
                }
            })
            .catch((err) => {
                res.render('videos', { error: 'Axios could not get link. Check your internet connection.' });
            })
    });
...
function byteToDiskSpace(sizeInBytes) {
    ...
    if (sizeInBytes > 9.99999999999999e14) return 'Video size is too large. Download a video less than 909TB';
